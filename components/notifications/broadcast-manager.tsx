...
import { Megaphone, Send, Loader2, CheckCircle2, Bell, Search, Users } from "lucide-react"
...
export default function BroadcastManager({ user }: { user: any }) {
  ...
  useEffect(() => {
    ...
    fetch("/api/voluntarios?status=activo")
      ...
      .then((data: any[]) => {
        ...
        // Ordenados por nombre para que sea cómodo de recorrer.
  ...
  async function handleSend() {
    ...
    setSending(true)
    try {
      // Si están todos seleccionados, mandamos por audiencia (alcanza también a
      // voluntarios que se agreguen después). Si es una selección parcial, los IDs.
      // El popup (announcement) es por audiencia, no por persona: solo aplica
      // cuando el envío es a todos.
      const payload: any = { title, body, audience: "voluntario", also_popup: allSelected && alsoPopup }
      if (!allSelected) payload.volunteer_ids = Array.from(selected)
      ...
      toast({
        ...
        description: `${data.recipients} destinatario(s) · ${data.push_sent} push a dispositivos`,
      })
      setTitle("")
      setBody("")
      setAlsoPopup(false)
  ...
  const canSend = title.trim().length > 0 && selected.size > 0 && !sending

  const recipientLabel = allSelected
          ...
          <div className="space-y-2">
            ...
            <div className="border-2 border-[#b2ebf2] rounded-lg overflow-hidden shadow-sm">
              ...
              <div className="relative">
                ...
                {/* Degradé inferior: pista de que hay más para scrollear */}
          ...
          {lastResult && !sending && (
            <Alert className="border-green-200 bg-green-50">
              ...
              <AlertDescription className="text-green-700">
                Último envío: {lastResult.recipients} destinatario(s), {lastResult.push_sent} push a dispositivos.
              </AlertDescription>
            </Alert>
          )}
        </CardContent>
      ...
      <AlertDialog open={confirmOpen} onOpenChange={setConfirmOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Se enviará a <strong>{recipientLabel}</strong>. Esta acción no se puede deshacer.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
